...
public class Optimizer : MonoBehaviour
{
    ...
    void Start()
    {
        StoppingFitness = float.MaxValue;
    ...
    {
        GameObject obj = Instantiate(Unit, startPosition.transform.position, startPosition.transform.rotation) as GameObject;
    ...
    public void RunBest()
    {
        ...
        FindObjectOfType<CameraHandler>().SetMainCameraOnObject(obj);
    }
    ...
    void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(10, 160, 100, 40), "Run SB Car"))
        {
            StartSbCar();
        }
        if (GUI.Button(new Rect(10, 210, 100, 40), "Reset"))
        {
            ResetWorld();
        }

        GUI.Button(new Rect(10, Screen.height - 70, 200, 60), string.Format("Generation: {0}\nFitness: {1:0.00}", Generation, Fitness));
    }

    private void StartSbCar()
    {
        FindObjectOfType<SbExecutor>().StartCar();
    }

    private void ResetWorld()
    {
        FindObjectOfType<CameraHandler>().ResetMainCamera();
